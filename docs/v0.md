...
| STATE_SNAPSHOT | GossipSub `/valence/peers` | §5 |
...
Peers MUST NOT trigger partition detection (§12) based on Merkle divergence with nodes whose `sync_status` is `"syncing"` — newly joining nodes will naturally diverge until sync completes.

#### Sync Phases

Reconciliation proceeds in priority order. Phases 1, 2, and 3 MUST complete in that order. Phases 4 and 5 MAY proceed in parallel after phase 3 completes. During sync, the node MUST NOT cast votes, create proposals, or issue flags. It MUST buffer live gossip messages received during each phase and apply them only after the corresponding phase's sync data has been processed — this prevents race conditions where live gossip (e.g., a DID_LINK) is processed before historical sync data (e.g., a DID_REVOKE with an earlier timestamp) that should take precedence. Buffered messages are applied in timestamp order after each phase completes.
...
- Nodes with reputation ≥ 0.5 SHOULD periodically publish **signed state snapshots** — a compact summary of current identity graph, reputation state, and active proposal set, signed by the publishing node.
- Snapshots are identified by `(publisher_node_id, timestamp)` and include Merkle roots for identity, reputation, and proposal state.
- A syncing node MAY bootstrap from a snapshot if it receives the **same snapshot content (matching Merkle roots) from ≥3 snapshot publishers across ≥3 ASNs**. This provides stronger Byzantine tolerance than replaying sync data from fewer peers.
- After snapshot bootstrap, the node performs incremental sync from the snapshot's timestamp to catch up on messages published since the snapshot.
- For reputation specifically, syncing nodes MAY request **current reputation state** (the computed reputation for each known node) rather than replaying all historical REPUTATION_GOSSIP messages. Current-state sync uses a new `SYNC_REQUEST` type filter: `"REPUTATION_CURRENT"`. The response contains `(node_id, reputation, last_assessment_timestamp)` tuples. This is validated against multiple peers — significant divergence (>0.1 for the same subject across peers) is flagged and the lower value used.
...
- **Graduated exit:** If phases 1–3 complete but phases 4–5 fail, the node MAY vote and propose (it has identity, reputation, and governance state) but MUST NOT participate in content flagging or storage decisions
- **Time limit:** After **7 days** in full degraded mode (phases 1–3 incomplete), the node MAY participate with **50% vote weight** (applied locally — the node halves its own vote's `vote_time_reputation` field). This prevents permanent governance exclusion due to transient sync failures. The reduced weight is visible to other nodes and signals degraded confidence.
...
Incremental sync covers phases 1–4. **Identity messages (phase 1) are included** in every 4th incremental sync cycle (i.e., approximately once per hour). This catches DID_REVOKE messages that may have been missed via live gossip, reducing the worst-case revocation gap from 30 days (re-broadcast interval) to ~1 hour.

#### Sync Serving Incentive

Serving sync requests is a network-health contribution. Nodes that serve sync requests have this counted as an uptime activity signal — specifically, each sync response served counts as a peer interaction for the purposes of uptime reward calculation (§9). This provides a small but non-zero incentive aligned with existing reputation mechanics, without creating a new reward category that could be farmed.

Nodes that refuse sync requests from >50% of requesting peers (measured over a rolling 24-hour window) SHOULD be penalized in peer scoring — other nodes MAY reduce their reputation weight in local calculations, similar to the treatment of clock-drifted nodes (§2).
...
| Incremental sync identity phase | every 4th cycle (~1 hour) | §5 |
| Degraded mode time limit | 7 days → 50% vote weight | §5 |
