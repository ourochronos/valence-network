...
1. **Minimum voters:** At least **3** distinct nodes have voted (absolute minimum, not relative to peer count)
2. **Quorum met:** Total reputation weight of all voters ≥ **1.5** (ensures meaningful participation regardless of network size)
3. **Threshold met:**
   ```
...
- The penalty applies to the **node's identity**, not just proposals. Key rotation (§1) resets tenure — this is an acceptable trade-off since key rotation sacrifices the old key permanently
...
- Higher quorum weight (suggested: minimum total voter reputation ≥ **3.0**)
...
| Minimum voters | 3 | §7 |
| Minimum quorum weight | 1.5 total reputation | §7 |
...
| Protocol change quorum weight | 3.0 total reputation | §12 |
