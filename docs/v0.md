...
SHARD_ASSIGNMENT is broadcast on `/valence/proposals`. Only the identity that submitted the REPLICATE_REQUEST may broadcast it. Uploaders SHOULD distribute shards across providers from diverse ASNs (minimum 3 distinct ASNs when possible) and SHOULD prefer providers with higher reputation. Providers who broadcast REPLICATE_ACCEPT but are not assigned simply never receive shards — no notification is required. Assigned providers who fail the first storage challenge within 7 days of assignment receive a small penalty (**-0.002**) to discourage accept-and-abandon behavior. The uploader MAY reassign shards from non-responsive providers by broadcasting an updated SHARD_ASSIGNMENT.
...
| Accept-and-abandon penalty | -0.002 (first challenge fail within 7 days) | §6 |
