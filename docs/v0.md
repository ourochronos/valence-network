...
**≥ 32 nodes:** Standard (Tier 1) governance activates. The network can propose and ratify non-constitutional changes — feature proposals, process improvements, new network services.

**≥ 1,024 nodes:** Constitutional (Tier 2) governance activates. The network can now modify its own foundations — but only with overwhelming consensus (0.90 threshold, 3× quorum, 90-day vote, 30-day cooling period).

**Post-critical mass (≥ 1,024 nodes):** The network is real. Full governance activates. Constitutional tier unlocks.

- Quorum: `max(5.0, min(total_known_reputation * 0.10, 50.0))`
...
- **Quorum:** 3× standard quorum weight
...
| Storage verified via hash challenge | +0.001 per challenge passed | Nodes storing shards are periodically challenged |
...
| Failed storage challenge | -0.01 | Claimed to store shard but couldn't produce it |
...
A **voting cycle** is a rolling 30-day window. A node is "active in a cycle" if any of its proposals were ratified during that window.

Nodes active in 4+ consecutive cycles receive a tenure penalty:
...
- Tenure tracks the **identity's history**, not the key. Key rotation (§1) transfers tenure along with reputation — rotating keys does NOT reset tenure. The new key inherits the old key's full cycle count
...
- 0.90 threshold, 3× quorum, 90-day minimum voting, 30-day cooling period
...
- **Storage verification limits:** A node can pass storage challenges by fetching on-demand from peers rather than actually storing shards. Mitigated by adding latency measurement to challenges (honest storage responds in <100ms, on-demand fetching takes longer).
...
| Minimum voters | 3 | §7 |
| Standard governance threshold | 32 nodes | §7 |
| Constitutional governance threshold | 1,024 nodes | §7 |
| < 32 nodes | frozen (content only) | §7 |
| Post-critical quorum | max(5.0, min(total_rep × 0.10, 50.0)) | §7 |
| Standard threshold | 0.67 | §7 |
| Constitutional threshold | 0.90 | §7 |
| Constitutional quorum | 3× standard | §7 |
