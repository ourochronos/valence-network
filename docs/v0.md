...
- One vote per node per proposal. A second vote from the same `from` key supersedes the first. For identities with authorized keys (§1 Identity Linking), all keys share one vote per proposal. A vote from any key is attributed to the root identity. The standard supersession rule applies — a later vote from any key in the identity replaces the earlier one.
