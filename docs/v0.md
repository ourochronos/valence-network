...
### Adaptive Quorum Weight

The quorum weight scales with the network:

```
quorum_weight = max(0.5, min(total_known_reputation * 0.10, 50.0))
```

- **Floor: 0.5** — allows a single founding node to ratify during the first minutes of bootstrap (when only 1-2 nodes exist). This is intentional: the bootstrap phase requires trust in founders.
- **10% of total known reputation** — as the network grows, quorum requires meaningful participation. At 100 nodes (avg 0.5 rep, total ~50), quorum is 5.0 — requires ~10 average nodes voting.
- **Ceiling: 50.0** — at very large scale, quorum stops growing to prevent paralysis. At 10,000 nodes (total rep ~5000), quorum is 50.0 — requires ~100 average nodes.

For **protocol change proposals**, multiply by 2: `protocol_quorum = quorum_weight * 2`.

During bootstrap (< 10 nodes), the low quorum floor means founders have outsized power. This is an acknowledged and intentional trade-off — the alternative is a network that can't start.
...
- Higher quorum weight (2× standard quorum)
...
Should anything be immune from governance? Currently, 80% of vote weight can modify any aspect of the protocol, including removing the reputation floor, the VDF, or lowering the voting threshold itself. Options:

- **No constitution (current design):** The network is sovereign. If it votes to self-destruct, that's its right. This is philosophically consistent but operationally risky.
- **Minimal constitution:** A small set of invariants that require unanimity (or 95%+) to modify: reputation floor > 0, VDF or equivalent sybil resistance, voting threshold ≥ 50%, key rotation must exist.
- **Immutable kernel:** Certain properties can never be changed. The network evolves everything else.

This is a values question, not a technical one. It should be decided before v0 is finalized.
...
| Minimum voters | 3 | §7 |
| Quorum weight floor | 0.5 | §7 |
| Quorum weight formula | max(0.5, min(total_rep × 0.10, 50.0)) | §7 |
| Protocol change quorum | 2× standard quorum weight | §7, §12 |
