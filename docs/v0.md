...
Replication costs reputation. Useful content earns it back.

**Uploader pays:** When content is replicated, the uploader commits a reputation stake proportional to size:

```
stake = content_size_mib × 0.002
```

(2 MiB of content costs 0.004 reputation to replicate.) The stake is deducted from the uploader's reputation immediately. This is NOT a deposit — it is spent.

**Demand sustains:** Content that gets adopted/downloaded frequently generates reputation for its storage providers via challenges. Popular content is self-sustaining — nodes want to store it because challenges earn reputation. Unpopular content generates fewer challenges and nodes may choose to drop shards when space is needed.

**Decay:** Replicated content with no ADOPT messages and no download requests (CONTENT_REQUEST or SHARD_QUERY) for **90 days** is eligible for garbage collection. Storage nodes MAY drop shards for decayed content. This is a local decision — some nodes may retain everything, others may aggressively prune.

**Proposed content is exempt from decay.** Content referenced by an active (non-archived) proposal MUST NOT be garbage collected regardless of download activity. The proposal's governance status protects its content.
...
| Min rep to propose | 0.2 (starting rep) | §7 |
...
| Replication stake | content_size_mib × 0.002 | §6 |
| Base storage allowance (rep 0.2) | 10 MiB | §6 |
| Content decay threshold | 90 days without activity | §6 |
