...
**Pre-critical mass (< 1,024 nodes):** The protocol is frozen. v0 is v0. Content proposals (skills, configs, documents, code) work normally — that's what the network is for. But **protocol change proposals are rejected**. The network doesn't govern itself until there are enough participants to make governance meaningful.

This prevents a small group from capturing governance before the network can defend itself. You can share, collaborate, and build reputation. You can't change the rules until 1,024 nodes are present to have a real conversation about it.
...
Pre-critical-mass, governance is disabled entirely. v0's parameters are the parameters. Once 1,024 nodes are in the network, governance activates — all v0 parameters become constitutional and require Tier 2 consensus to change.
...
| Minimum voters | 3 | §7 |
| Critical mass | 1,024 nodes | §7 |
| Pre-critical governance | frozen (content only, no protocol changes) | §7 |
