...
| COPY_QUERY | stream `/valence/sync/1.0.0` | §6 |
| COPY_QUERY_RESPONSE | stream `/valence/sync/1.0.0` | §6 |
...
| COPY_ASSIGNMENT | GossipSub `/valence/proposals` | §6 |
| COPY_RECEIVED | GossipSub `/valence/proposals` | §6 |
...
| RENT_TRANSFER | GossipSub `/valence/proposals` | §6 |
| RENT_TRANSFER_ACK | GossipSub `/valence/proposals` | §6 |
...
### Quality of Service by Reputation

Peers prioritize serving higher-reputation nodes with better bandwidth, lower latency, and more available connection slots. This is **local peer behavior**, not protocol-level enforcement — nodes make their own decisions about resource allocation based on peer reputation.

**No bandwidth metering:** The protocol does NOT include per-peer bandwidth accounting, transit fees, or provable QoS guarantees. Instead, reputation serves as a natural quality-of-service signal: well-behaved, contributing nodes accumulate reputation and receive better service; penalized or fresh nodes receive baseline service.

**Why reputation-based QoS:** A full bandwidth economy (metered transit, proof-of-relay, payment routing) is an unsolved problem in P2P networks. Reputation-based prioritization provides a practical QoS mechanism for bootstrap scale without requiring complex bandwidth verification infrastructure. Future governance proposals MAY introduce more sophisticated bandwidth economics if operational experience demonstrates the need.
...
1. Exchange Merkle trees at increasing depth to identify divergent subtrees. The narrowing exchange uses SYNC_REQUEST with optional fields: `"merkle_tree": "identity" | "proposal"`, `"depth": <integer>`, `"subtree_path": [0|1, ...]` (binary path from root). SYNC_RESPONSE includes `"merkle_nodes": [{"path": [...], "hash": "<hex>"}]` for the requested subtree.
2. Request the specific message IDs that differ (identified by Merkle leaf comparison)
...
- **Graduated exit (phases 1–3 complete):** If phases 1–3 complete but phases 4–5 fail, the node MAY vote and propose (it has identity, reputation, and governance state) but MUST NOT participate in content flagging or storage decisions. This node uses `sync_status: "degraded"` and carries the same 50% weight as any degraded node. The uniform 50% penalty for all degraded states (regardless of which phases completed) is a deliberate simplification — finer-grained sync states would proliferate interaction effects with governance. Nodes with phases 1–3 have full context for informed voting; the 50% penalty reflects the residual risk of incomplete network state, not judgment quality.
- **Time-limited exit (phases 1–3 incomplete):** After **7 days** in full degraded mode where phases 1–3 have NOT completed, the node remains unable to vote or propose. There is no automatic governance participation with incomplete identity or reputation state — the risk of confused voting outweighs the cost of temporary exclusion. The node MUST continue retry attempts. If phases 1–2 complete during degraded mode (even if phase 3 hasn't), the node MAY vote with **50% weight** — it has identity and reputation context even if its proposal knowledge is incomplete. The VOTE message itself MUST include the voter's `sync_status` at vote creation time. Other nodes apply `min(self_declared_weight, weight_from_observed_sync_status)` — the 50% reduction applies if EITHER the vote declares degraded OR the node independently observes the voter as degraded via PEER_ANNOUNCE. This handles gossip lag in both directions. During cold start (§8), degraded nodes count as full participants for headcount purposes (they are active nodes) but their votes count as **0.5 votes** in the endorsement calculation. Cold start uses headcount (no reputation weighting), but the 50% degraded-mode reduction is sync-status-based, not reputation-based, and MUST still apply — a node with incomplete state should not have full governance influence regardless of voting mode. Example: 10 nodes vote (7 synced + 3 degraded). Effective vote count for endorsement: 7 + 1.5 = 8.5. Endorsement threshold (>67%): requires >5.7 effective endorsement votes.
...
A node requests the network to store content durably as full copies distributed across multiple providers. Content survives the uploader going offline.
...
```json
{
  ...
  "payload": {
    ...
    "description": "<optional string>",
    "tags": ["<string>", ...],
    "copy_count": <integer — 1 to 5, default 3>,
...
**File size cap:** Content size MUST NOT exceed **100 MB**. This limit MAY be adjusted via governance proposal as network capacity grows.

The `reputation_stake` field is the uploader's committed budget for the first billing cycle's rent. Daily rent is deducted from the uploader's reputation directly (not drawn from this stake). The stake serves as a signal that the uploader can afford at least one cycle of rent at current pricing.

Storage providers signal willingness to store full copies via REPLICATE_ACCEPT:
...
REPLICATE_ACCEPT is broadcast on `/valence/proposals`. The uploader collects acceptances and distributes full copies to providers via direct stream on `/valence/content/1.0.0`. If fewer than `copy_count` providers accept within **48 hours**, the replication fails and the `reputation_stake` is returned. Providers MUST have the `store` capability and `available_bytes` ≥ content size.

After collecting sufficient acceptances, the uploader broadcasts a COPY_ASSIGNMENT to confirm the distribution:
...
```json
{
  ...
  "payload": {
    "content_hash": "<sha256_hex>",
    "providers": ["<hex_node_id>", "<hex_node_id>", ...]
  }
}
```

COPY_ASSIGNMENT is broadcast on `/valence/proposals`. Only the identity that submitted the REPLICATE_REQUEST may broadcast it. **Provider confirmation:** Assigned providers MUST broadcast a COPY_RECEIVED message confirming they hold the full copy before the assignment is considered valid by the network:
...
COPY_RECEIVED is broadcast on `/valence/proposals`. Nodes SHOULD NOT consider a copy assignment complete until the corresponding COPY_RECEIVED is observed. Uploaders SHOULD distribute copies across providers from diverse ASNs (minimum 3 distinct ASNs when possible) and SHOULD prefer providers with higher reputation. Providers who broadcast REPLICATE_ACCEPT but are not assigned simply never receive copies — no notification is required. Assigned providers who fail the first storage challenge within 7 days of assignment AND have broadcast COPY_RECEIVED receive a penalty (**-0.002**) for accept-and-abandon. Providers listed in COPY_ASSIGNMENT who never broadcast COPY_RECEIVED are not penalized — the uploader may have failed to deliver. If fewer than `copy_count` providers confirm receipt within 7 days, the replication is considered failed. The uploader MAY reassign copies to new providers by broadcasting an updated COPY_ASSIGNMENT.

**Properties:**
- Costs reputation proportional to content size
- Durable — full copies across willing storage peers
...
This gives the network a supply signal. Nodes choose how much disk to allocate — there is no minimum. Nodes with `"store"` capability and available space are eligible to receive copies.

**Capacity claim verification:** Storage capacity self-reports are unverifiable directly, but nodes SHOULD detect outliers: if a node claims 1 TB allocated but stores < 10 MB of actual content (as observed via content discovery), the claim is suspicious. Nodes SHOULD weight capacity reports by the reporter's reputation when evaluating provider selection — fresh nodes' capacity claims carry less weight.
...
Each node reports `allocated_bytes` and `available_bytes` in PEER_ANNOUNCE to provide a supply signal. Storage pricing uses a **flat rate** per GB per day, set at genesis and adjustable via governance proposal.

**Flat rate pricing simplifies the storage market for early network operation.** Future governance proposals MAY introduce scarcity-based pricing (utilization curves, moving averages) when the network is large enough for capacity constraints to matter meaningfully.

#### Replication Rent

Replicated content has an ongoing daily cost:

```
daily_rent = content_size_gb × base_rate
base_rate = [TBD — set at genesis, adjustable via governance] per GB per day
```

The flat rate keeps storage economics simple and predictable during early network operation. Future governance proposals MAY introduce dynamic pricing based on network utilization when capacity becomes scarce.

Rent is deducted from the uploader's reputation at the start of each **24-hour billing cycle** (computed from the content's replication timestamp). The deducted reputation is distributed to storage providers proportional to copies held (see Storage Challenges below).

**If the uploader cannot pay rent** (insufficient reputation), the content enters a **7-day grace period**. During the grace period, storage providers are not paid but SHOULD continue holding copies. After the grace period, content is eligible for garbage collection — storage nodes MAY drop copies. Seven days is sufficient for a healthy node to earn enough reputation to resume payment (max daily gain 0.02, weekly 0.14) without enabling systematic rent evasion. Storage providers are NOT compensated during grace periods. Providers who continue holding copies do so without payment and may rationally choose to drop copies at grace period entry. Uploaders should not rely on copy survival through grace periods — content may become irretrievable even before the grace period expires. **Repeat failures:** If the same content enters grace period more than once within 90 days, the second grace period is 3 days, and the third is 0 days (immediate GC eligibility).

#### Storage Provider and Validator Revenue

Storage rent is split between **providers** (nodes holding copies) and **validators** (nodes issuing challenges):

- **Providers receive 80%** of rent for content they store, distributed equally among providers holding copies.
- **Validators receive 20%** of rent, distributed proportional to challenges issued and verified during the billing cycle.

Validators are any node that holds a full copy of the content and broadcasts a valid CHALLENGE_RESULT. Validating is economically incentivized — nodes earn reputation for keeping providers honest.

**Minimum challenge requirements:** Providers MUST pass at least **10 challenges from at least 2 distinct validators** per billing cycle to receive full rent. Rent is pro-rated by challenges passed: a provider who passes 5 of 10 minimum challenges receives 50% of their provider share. Providers who pass zero challenges receive nothing and pay the failed challenge penalty (see Stacking Storage Penalties below).
...
Validators MUST NOT validate their own copies — CHALLENGE_RESULT messages where the challenger and challenged are the same identity MUST be rejected.
...
```json
{
  ...
  "payload": {
    ...
    "providers": [
      {"node_id": "<hex>", "amount": <fixed_point>}
    ]
  }
}
```

RENT_PAYMENT is broadcast on `/valence/proposals` by the content uploader at the start of each billing cycle. **RENT_PAYMENT is a coordination signal, not an authoritative transfer.** Each node MUST independently compute rent obligations and reputation adjustments from: (a) the REPLICATE_REQUEST's replication timestamp, (b) the content size and flat rate, (c) the list of providers holding copies, and (d) which providers passed challenges during the billing cycle. Nodes that observe divergence between a RENT_PAYMENT and their own calculation MUST trust their own calculation. The message exists to assist coordination and provide an audit trail, not to dictate reputation changes.
...
```json
{
  ...
  "payload": {
    "content_hash": "<sha256_hex>",
    "receiving_provider": "<hex_node_id>",
    "transfer_fee": <fixed_point — small rep cost, [TBD — economics pass required]>
  }
}
```

RENT_TRANSFER is broadcast on `/valence/proposals` by the current provider. The receiving provider MUST confirm acceptance:

```json
{
  "type": "RENT_TRANSFER_ACK",
  "payload": {
    "content_hash": "<sha256_hex>",
    "original_provider": "<hex_node_id>",
    "transfer_id": "<message_id of RENT_TRANSFER>"
  }
}
```

RENT_TRANSFER_ACK is broadcast on `/valence/proposals`. **Transfer is only valid after content transmission:** The receiving provider MUST hold a complete copy of the content (verifiable via storage challenge) before broadcasting RENT_TRANSFER_ACK. Nodes SHOULD NOT consider the transfer complete until a RENT_TRANSFER_ACK is observed AND the receiving provider has passed at least one storage challenge for the content.

**Transfer fee:** The initiating provider pays a small reputation cost (deducted on RENT_TRANSFER broadcast). This fee MUST be significantly smaller than the penalty for failed storage challenges, creating a strong incentive to transfer rather than abandon. The specific value is TBD pending economics modeling.

After a successful transfer, future rent payments and storage challenges apply to the receiving provider. The original provider is released from storage obligations for that content.
...
On receipt, storage providers SHOULD drop copies within 24 hours of `effective_after`. No further rent is charged. Content referenced by ANY active proposal (any proposal whose `voting_deadline` has not yet passed) cannot be withdrawn — the CONTENT_WITHDRAW MUST be rejected until all referencing proposals conclude.
...
Content is stored as full copies distributed across multiple providers. The consumer specifies the desired number of copies (1-5, default 3) in the REPLICATE_REQUEST.

**Why full copies for v0:** Erasure coding (Reed-Solomon, shard coordination, reconstruction logic) adds significant complexity. At bootstrap scale (20-50 nodes), full copies provide sufficient redundancy with simpler verification and retrieval. Erasure coding MAY be introduced via governance proposal when file sizes grow and storage efficiency becomes critical.

**Verification:** Storage challenges verify that providers hold the complete content by requesting hash proofs of random byte ranges (see Storage Challenges below). Providers MUST hold the full content — lazy reconstruction on challenge is detectable via timing analysis.

### Storage Challenges

Storage challenges use adjacency proofs rather than full content retrieval. A challenger picks a random offset within the content and asks: "What is the hash of the N bytes before/after this offset?" This requires the node to actually hold the full content — it can't answer by fetching on demand because it doesn't know which offset will be challenged.

```json
{"type": "STORAGE_CHALLENGE", "payload": {"content_hash": "<hex>", "offset": <bytes>, "direction": "before | after", "window_size": <bytes — default 4096, range 1024..65536>, "challenge_nonce": "<hex — random 32 bytes>"}}
{"type": "STORAGE_PROOF", "payload": {"proof_hash": "<SHA256(challenge_nonce || window_bytes)>"}}
```

The challenger verifies the proof against their own copy of the content. **Challengers MUST hold a full copy** of the content to issue a valid challenge.
...
```json
{"type": "CHALLENGE_RESULT", "payload": {"challenge_id": "<message_id of STORAGE_CHALLENGE>", "content_hash": "<hex>", "challenged_node": "<hex_public_key>", "result": "pass | fail"}}
```

Third-party nodes track challenge success rates per (challenger, challenged) pair. Suspiciously high pass rates between the same pair (>20 consecutive passes with no failures across all content) SHOULD trigger collusion review. Challenges MUST be issued randomly by peers who hold the same content. Frequency SHOULD be approximately once per copy per day to meet the minimum challenge requirements (10 per cycle from ≥2 validators). Validators earn a share of storage rent for each verified challenge (see Storage Provider and Validator Revenue above). Nodes MUST NOT issue more than 10 challenges to the same peer per day.

### Copy Discovery

Nodes that store content announce this via the sync protocol:

```json
{"type": "COPY_QUERY", "payload": {"content_hash": "<hex>"}}
{"type": "COPY_QUERY_RESPONSE", "payload": {"has_copy": <boolean>}}
```

Nodes MUST rate-limit COPY_QUERY messages to 10 per sender per minute.
...
The protocol does not mandate copy replication monitoring in v0. Nodes that store content SHOULD periodically verify that sufficient copies remain available across known peers (at least `copy_count` as specified in the original REPLICATE_REQUEST). When the number of active copies drops below the requested count, nodes SHOULD coordinate re-distribution to willing providers.
...
- `tags`: Array of strings, max 10 tags, each max 64 chars, lowercase. Optional but recommended for discoverability.
- `description`: String, max 1 KiB, plain text or markdown. Optional. Provides context for evaluation and search.
...
Metadata standards are proposable content. Good standards get adopted through stigmergy. Bad ones fade.

**Content delivery:** Small artifacts (< 1 MiB) MAY be inlined as base64 in `content_inline`. The 1 MiB limit applies to the original artifact size before base64 encoding. Larger artifacts are referenced by `content_hash` and distributed as full copies (see §6 Full-Copy Replication). The hash ensures integrity regardless of source.
...
**Protocol change proposals:** Nodes SHOULD apply heightened scrutiny to protocol change proposals (tier: constitutional). While v0 does not enforce specific replication requirements beyond the 100MB size cap, proposers SHOULD use higher copy counts (4-5) for protocol changes to ensure availability during the extended voting period.
...
```json
{
  ...
  "payload": {
    ...
    "reason": "<optional string>",
    "sync_status": "<syncing | degraded | synced — voter's sync status at vote creation time, see §5>"
  }
}
```
...
| Uptime (continuous availability) | +0.000333 per day | Caps at 30 days accumulation. The baseline reward for being a reliable network participant. |
...
| Failed storage challenge | Escalating (see Stacking Storage Penalties below) | Penalties stack in a 7-day rolling window |
| Collusion detected | -0.05 | See §11 |
| Inactivity | -0.000667 per day | After 30 days inactive. A node is "active" if it has authored and broadcast at least one signed protocol message (VOTE, PROPOSE, ADOPT, STORAGE_PROOF, or PEER_ANNOUNCE) within the window. PEER_ANNOUNCE alone suffices — it proves liveness without requiring governance participation. |
...
### Stacking Storage Penalties

Failed storage challenges result in escalating penalties within a **7-day rolling window**:

| Failure # in window | Penalty | Notes |
|---------------------|---------|-------|
| 1st | [TBD — small, economics pass required] | Honest failures happen — power outages, network issues |
| 2nd | [TBD — larger] | Pattern emerging |
| 3rd | [TBD — significant] | Clear reliability problem |
| 4th+ | [TBD — harsh] | Systematic abandonment |

**Window reset:** As challenges are passed, the failure window gradually clears. The window is a rolling 7-day count of failures.

**Timeout vs wrong proof:** Future refinement MAY distinguish between timeout (node offline, recoverable) and wrong proof (data loss, more severe). For v0, both are treated identically pending operational experience.

**Economics pass required:** Specific penalty values are TBD. These must be modeled against real scenarios (home user missing 1 challenge/week, provider down 4 hours, etc.) before setting concrete numbers. The mechanism is established; the values will be calibrated based on network behavior modeling.

**Why stacking penalties:** A flat penalty per failure is catastrophic for honest providers experiencing temporary issues. A power outage that causes 3 missed challenges shouldn't destroy months of earned reputation. Stacking penalties start gentle (honest failures are cheap) and escalate sharply (abandonment is expensive).
...
**Evaluation order (fixed-point):** All intermediate values use 64-bit scaled integers (×1,000,000). The formula evaluates as:
```
peer_sum = Σ(trust_i × assessment_i)     // each already ×1,000,000, product is ×10^12
peer_weight = Σ(trust_i)                  // ×1,000,000
peer_avg = peer_sum / peer_weight         // back to ×1,000,000 scale
result = (α × direct + (1000000 - α) × peer_avg) / 1000000
```
...
### General-Purpose Banning

v0 has a content-triggered DID ban mechanism (§6 — illegal content flagging can result in permanent DID ban via standard governance at 0.67 threshold). However, there is no general-purpose mechanism to permanently exclude a node for reasons unrelated to content. A node with reputation at floor (0.1) can still participate, just with minimal weight. This is intentional — permanent exclusion is antithetical to the protocol's values. Nodes can locally block specific peers at the implementation level. The network can propose a broader ban mechanism if it decides it needs one.
...
### Conformance Test Updates Required

The following protocol changes invalidate existing conformance tests and require new test suites:

- **Fixed-point precision change (×10,000 → ×1,000,000):** All numeric serialization tests, reputation calculation tests, and fixed-point arithmetic tests need updated expected values.
- **24-hour billing cycles:** Storage rent calculation tests, grace period timing tests, and rent payment schedule tests need adjusted time windows.
- **Full copies vs erasure coding:** All shard-based tests (encoding, reconstruction, challenge verification) must be replaced with full-copy verification tests.
- **Stacking storage penalties:** Penalty calculation tests need multi-failure scenarios in rolling windows rather than flat penalty assertions.
- **Message type changes:** Tests referencing SHARD_ASSIGNMENT, SHARD_RECEIVED, SHARD_QUERY, SHARD_QUERY_RESPONSE must use COPY_ASSIGNMENT, COPY_RECEIVED, COPY_QUERY, COPY_QUERY_RESPONSE.
- **RENT_TRANSFER flow:** New message types require new test coverage for transfer initiation, acknowledgment, and failure cases.

These changes do not affect the conformance test framework itself, only the test cases and expected values.
...
| Daily inactivity decay | 0.000667 | §9 |
...
| Uptime reward | +0.000333 per day (caps at 30 days accumulation) | §9 |
...
| Numeric precision | ×1,000,000 fixed-point integers | §2 |
...
| Default copy count | 3 (range 1-5) | §6 |
...
| Storage rent base rate | [TBD] per GB per day | §6 |
| Storage rent billing cycle | 24 hours | §6 |
...
| Accept-and-abandon penalty | -0.002 (challenge fail within 7 days + COPY_RECEIVED sent) | §6 |
...
| PROPOSE tags limit | max 10 tags, max 64 chars each, lowercase | §7 |
| PROPOSE description limit | max 1 KiB | §7 |
| Replicated content size cap | 100 MB | §6 |
| Rent transfer fee | [TBD — small, economics pass required] | §6 |
| Stacking storage penalties | escalating in 7-day window [TBD — economics pass required] | §6, §9 |
...
| REPUTATION_CURRENT integrity | trimmed minimum (discard high/low, min of middle 3); fallback when 2nd-highest − 2nd-lowest > 0.1; sparse: 4→trim 1, 3→min, ≤2→full replay | §5 |
