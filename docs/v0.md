...
**Capability gate:** Provenance credit recipients MUST have reputation ≥ 0.3 to receive their share of adoption rewards. Below 0.3, the proposer receives 100% regardless of `origin_share_id`. This prevents colluding identities from using provenance splits to bypass the capability ramp.

**Anti-gaming:** Provenance is a reward split, not reward creation. A colluding ring farming provenance credit earns the same total reputation as one without it — the split just moves rewards between ring members. Additionally, provenance recipients must meet the 0.3 capability threshold, preventing the split from being used to bootstrap fresh identities past the reputation ramp. The farming path is slower and more expensive than the legitimate path (storing shards and earning rent).
...
**Rent price locking:** The scarcity multiplier used for rent calculation is locked at the time of replication (the multiplier in effect when the REPLICATE_REQUEST is accepted). Rent obligations do not change when the network-wide scarcity multiplier changes. New replications price at the current multiplier; existing rent obligations remain stable. This prevents cascading failures where rising scarcity makes existing content unpayable, triggering mass grace periods and a death spiral.
...
```json
{
  ...
  "payload": {
    ...
    "locked_multiplier": <fixed_point — scarcity multiplier at replication time>,
    "providers": [
...
RENT_PAYMENT is broadcast on `/valence/proposals` by the content uploader at the start of each billing cycle. Storage providers verify the amount against the locked multiplier and their shard count. Providers who passed at least one challenge during the previous cycle accept the payment; those who failed receive nothing. Third-party nodes track rent payments as part of the economic audit trail.
...
CONTENT_WITHDRAW is broadcast on `/valence/proposals`. Only the identity that submitted the REPLICATE_REQUEST may withdraw. On receipt, storage providers SHOULD drop shards within 24 hours. No further rent is charged. Content referenced by an active proposal (voting period not expired) cannot be withdrawn — the CONTENT_WITHDRAW MUST be rejected until the proposal concludes.
...
| Provenance credit min rep | 0.3 | §6 |
| Rent price locking | at replication time | §6 |
