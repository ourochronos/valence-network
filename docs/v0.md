...
**Content delivery:** Small artifacts (< 1 MiB) MAY be inlined as base64 in `content_inline`. Larger artifacts are referenced by `content_hash` and fetched via `content_url` or from peers. The hash ensures integrity.
...
| Action | Reward | Notes |
|---|---|---|
| Proposal adopted by peers | +0.005 per adopting node (observed via gossip) | Capped at +0.05 per proposal |
...
### Reputation Adoption Tracking

"Proposal adopted by peers" rewards adoption, but there's no `ADOPT` message type. Options:
- Add an `ADOPT` message type (adds protocol complexity)
- Infer adoption from behavior (unreliable)
- Drop adoption-based reputation and rely only on verification-based rewards
- Track adoption implicitly via references (proposals that `supersede` or cite other proposals)
...
The Merkle root covers all active proposals. Without pruning, this set grows unbounded. Need a garbage collection strategy:
- Proposals older than X days auto-expire?
- Adopted proposals move to a separate archive set?
- Only track proposals from the last N voting cycles?

### VDF Hardware Heterogeneity

A 30-second VDF on commodity hardware is 3 seconds on a fast server. The difficulty parameter doesn't equalize across hardware. This means well-resourced attackers face a lower cost per identity. Mitigations:
- Accept this as a known trade-off (it raises the floor, not the ceiling)
- Propose a proper VDF with verifiable timing in v1+

### Blacklisting / Banning

v0 has no mechanism to permanently exclude a node. A node with reputation at floor (0.1) can still participate, just with minimal weight. Options:
- Intentional — permanent exclusion is antithetical to the protocol's values
- Propose a ban mechanism if the network decides it needs one
- Nodes can locally block specific peers (already possible at the implementation level)

### Content Delivery for Large Artifacts

Proposals reference large artifacts by hash but `content_url` depends on external hosting. Peer-to-peer content delivery (BitTorrent-style shard distribution) should be proposed as a network service.
