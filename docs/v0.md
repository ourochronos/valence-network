...
**Pre-critical mass (< 1,024 nodes):** Bootstrap phase. Founders run the show. Governance is informal — quorum is low, all proposals are standard tier. Move fast, build the thing. The network isn't a democracy yet; it's a construction site.

- Quorum: `max(0.5, total_known_reputation * 0.05)`
- All proposals use standard threshold (0.67)
- No constitutional tier (nothing to constitutionalize yet)
...
Pre-critical-mass, the constitutional tier doesn't exist. Founders can set the initial parameters however they see fit. Once 1,024 nodes are in the network, those parameters become constitutional and require Tier 2 consensus to change.
...
- Pre-critical-mass: protocol changes are standard tier (0.67 threshold)
...
| Pre-critical quorum | max(0.5, total_rep × 0.05) | §7 |
