...
All activity_multiplier inputs are derived from publicly observable protocol messages (ADOPT counts, reputation deltas, STORAGE_CHALLENGE/STORAGE_PROOF pairs), not from local trust computation. This ensures governance activation timing depends only on objective, verifiable signals.
