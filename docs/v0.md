...
The challenger verifies the proof against their own copy of the shard (or by requesting the same window from another storage node and comparing). Challenges SHOULD be issued randomly by peers, suggested frequency: once per shard per day.
...
The network operates in two phases based on the number of distinct nodes with valid VDF proofs:

The network has two critical mass thresholds:

**< 32 nodes:** The protocol is frozen. Content proposals (skills, configs, documents, code) work — that's what the network is for. But **all governance proposals are rejected**. Share, collaborate, build reputation. No rule changes.

**≥ 32 nodes for 7 consecutive days:** Standard (Tier 1) governance activates. The network can propose and ratify non-constitutional changes — feature proposals, process improvements, new network services. The 7-day sustain requirement prevents governance ambush — an attacker can't spin up nodes, pass proposals, and leave. The network must demonstrate stable participation before governance unlocks.

**≥ 1,024 nodes for 30 consecutive days:** Constitutional (Tier 2) governance activates. The network can now modify its own foundations — but only with overwhelming consensus (0.90 threshold, 30% quorum, 90-day vote, 30-day cooling period). The 30-day sustain requirement prevents Sybil-driven threshold gaming.

**Post-critical mass (≥ 1,024 nodes):** The network is real. Full governance activates. Constitutional tier unlocks.

- Standard quorum: `max(5.0, total_known_reputation * 0.10)` (no ceiling — scales with network)
- Constitutional quorum: `total_known_reputation * 0.30` (30% of the network's reputation must participate)
- Two proposal tiers with different thresholds (see below)
...
| Limit | Value | Applies |
|---|---|---|
| Maximum daily gain | 0.02 | Above 0.5 (starting score) |
| Maximum weekly gain | 0.08 | Above 0.5 |

**Recovery below starting score is uncapped.** A node that has been penalized below 0.5 can recover at whatever rate their contributions earn, with no daily/weekly cap, until they reach 0.5 again. Above 0.5, velocity limits apply normally.

This prevents identity recycling: recovering from punishment is fast enough that abandoning an identity and re-registering at 0.5 offers no meaningful advantage. A punished node that actively contributes can return to 0.5 in days, not months. Gaining reputation *above* 0.5 still requires sustained contribution over time.
...
- Skipping a single cycle resets the counter — but costs 30 days of influence. The trade-off is real: you lose a month of voting to reset tenure
...
The constitutional threshold (0.90) is meaningless if turnout is low. 18 out of 1,024 nodes agreeing is not consensus — it's a quorum exploit. Constitutional proposals need real participation, not just a high approval percentage among the few who show up.

Open design questions:
- **Minimum voter participation:** Should constitutional proposals require a percentage of the network (e.g., 30% of known nodes) to cast a vote before the result counts?
- **Voting incentives:** Should there be a small reputation reward for voting on constitutional proposals, to drive turnout? Risk: incentivizing uninformed votes. But without it, participation may be too low for the threshold to mean anything.
- **Abstention as signal:** Should explicit abstention count toward participation quorum without affecting the endorse/reject ratio? This lets nodes say "I showed up but don't have a strong opinion" without being forced to pick a side.
...
| Minimum voters | 3 | §7 |
| Standard governance threshold | 32 nodes sustained 7 days | §7 |
| Constitutional governance threshold | 1,024 nodes sustained 30 days | §7 |
| < 32 nodes | frozen (content only) | §7 |
| Standard quorum | max(5.0, total_rep × 0.10) — no ceiling | §7 |
...
| Recovery below 0.5 | uncapped (no velocity limit) | §8 |
| Tenure penalty onset | 6 consecutive cycles | §10 |
