...
Reconciliation proceeds in strict priority order. Each phase MUST complete before the next begins. During sync, the node MUST NOT cast votes, create proposals, or issue flags. It MAY relay gossip messages it receives and SHOULD process them into local state.
...
| 5. Storage State | `REPLICATE_REQUEST`, `REPLICATE_ACCEPT`, `SHARD_ASSIGNMENT`, `SHARD_RECEIVED` | Only relevant if the node participates in storage |

Each phase uses the existing `SYNC_REQUEST` message with the `types` field set to that phase's message types. The `since_timestamp` SHOULD be the node's last known good timestamp (persisted to disk on clean shutdown) or `0` for first join.

#### Merkle-Based Divergence Detection

After completing phases 1–3, the node MUST compare its proposal Merkle root (§12) against at least **3 peers from ≥2 ASNs**. If all roots match, the node is consistent. If roots diverge:

1. Exchange Merkle trees at increasing depth to identify divergent subtrees
2. Request the specific proposal IDs that differ
3. Fetch missing proposals and their votes via `SYNC_REQUEST`
...
Nodes MUST sync from at least **3 peers** selected with ASN diversity (≥2 distinct ASNs, matching §4 anti-fragmentation requirements). This prevents a single malicious peer from feeding false state during sync.

When the same message is received from multiple sync peers, the node uses the **first valid instance** (signature-verified, schema-valid). Conflicting messages from different peers (e.g., different REPUTATION_GOSSIP assessments for the same subject) are all ingested — reputation computation already handles multiple assessors (§9).

For identity messages (phase 1), nodes SHOULD request the same identity chains from all 3 sync peers and verify consistency. A DID_REVOKE present on any peer MUST be treated as authoritative (revocation is irreversible).
...
- **Jitter:** Nodes MUST add random jitter of **0–30 seconds** before initiating sync after connection.
...
- **Sync bandwidth cap:** Nodes SHOULD limit sync traffic to **50% of available bandwidth**, preserving capacity for live gossip.
- **Staggered phases:** Nodes SHOULD wait **1–5 seconds** (random) between completing one sync phase and starting the next, to spread load across sync peers.

Sync peers MUST respect the existing rate limit of **10 SYNC_REQUEST messages per peer per minute** (§1). This bounds the maximum sync load any single peer imposes.

#### Sync Completeness

A node considers itself **sync-complete** when:

1. All 5 phases have completed (no `has_more: true` responses pending)
2. Its proposal Merkle root matches at least **2 of 3** sync peers
3. It has processed at least one `REPUTATION_GOSSIP` batch from each sync peer

After sync-complete, the node transitions to normal operation: it MAY vote, propose, flag, and participate in storage. The transition SHOULD be logged.

**Partial sync tolerance:** If a node cannot complete all phases (e.g., peer failures), it MAY operate in **degraded mode** — processing live gossip and building state incrementally. Nodes in degraded mode MUST NOT vote or propose (they lack reliable state for quorum evaluation) but MAY relay messages, serve storage challenges, and accumulate reputation via uptime.

#### Incremental Sync

During normal operation, nodes SHOULD periodically sync with a random peer (suggested: every **15 minutes**, aligned with reputation gossip interval). This catches messages missed due to transient network issues. Incremental sync uses the same `SYNC_REQUEST` mechanism but with `since_timestamp` set to `now - 30 minutes` (overlapping window to account for message propagation delay). Only phases 2–4 are needed for incremental sync — identity changes propagate via live gossip and the 30-day re-broadcast mechanism (§1).
...
| Sync phases | 5 (identity → reputation → proposals → content → storage) | §5 |
| Sync peer minimum | 3 peers from ≥2 ASNs | §5 |
| Sync jitter | 0–30 seconds | §5 |
| Sync backoff | 5s initial, 5m max (exponential) | §5 |
| Sync Merkle agreement | 2-of-3 peers | §5 |
| Incremental sync interval | 15 minutes | §5 |
| Incremental sync window | 30 minutes lookback | §5 |
