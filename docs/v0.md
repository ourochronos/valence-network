...
SHARE messages are broadcast on `/valence/peers`. Nodes SHOULD re-broadcast SHARE messages every **30 minutes** (consistent with peer expiry). When a node goes offline, its shared content becomes unavailable.
...
Replicated content wrapped in a PROPOSE message (§8). Full governance flow — votes, adoption, reputation consequences. Content MUST be replicated before it can be proposed. The proposal references the content by `content_hash`.
...
Examples:
- 20% utilized: multiplier = 1.25 (cheap)
- 50% utilized: multiplier = 2.0
- 80% utilized: multiplier = 5.0
- 95% utilized: multiplier = 20.0 (expensive)
...
**If the uploader cannot pay rent** (insufficient reputation), the content enters a **30-day grace period**. During the grace period, storage providers are not paid but SHOULD continue holding shards. After the grace period, content is eligible for garbage collection — storage nodes MAY drop shards. This gives uploaders time to earn reputation and resume payment.

#### Storage Provider Revenue

Storage providers earn reputation from rent payments, verified via the challenge system:

- When rent is collected, the total rent for a piece of content is divided among storage providers who pass at least one challenge during that billing cycle.
- Distribution is proportional to shards held: a node holding 3 of 8 shards receives 3/8 of the rent.
- Providers who fail a challenge during a billing cycle receive NO rent for that cycle AND pay the failed challenge penalty (-0.01).

This means storage challenges serve dual purpose: they verify data integrity AND authorize rent payment. No challenge passed = no revenue.
...
- **Proposed content is exempt from decay.** Content referenced by an active (non-archived) proposal MUST NOT be garbage collected regardless of rent status. The proposal's governance status protects its content — the network bears the storage cost as a public good. Providers holding shards for proposed content earn from the proposal adoption reward pool instead of rent (see §9 Reputation).
...
- Nodes that process the flag SHOULD immediately drop shards for the flagged content. Do not wait for consensus — err on the side of not hosting potentially illegal material.
- Content source DID is suspended pending governance vote (same suspension as KEY_CONFLICT: reputation frozen at floor, voting weight reduced to 10%).
...
COMMENT messages are broadcast on `/valence/proposals`. They are lightweight, signed, and attributed. Comments do not affect governance — they are signal for evaluation, not votes. Rate limit: **10 COMMENT messages per identity per hour.**
...
| Action | Reward | Notes |
|---|---|---|
| Proposal adopted by peers | +0.005 per ADOPT message received | Capped at +0.05 per proposal. The primary incentive for creating useful content. |
...
| Replication of proposed content | Network (adoption pool) | Storage providers | Share of proposal adoption rewards | Providers earn for storing governed content |
...
|---|---|
| 0.2 (starting) | Sync, browse, adopt, store shards (earn rent), uptime accrual |
...
| Storage rent grace period | 30 days | §6 |
...
| COMMENT rate limit | 10 per identity per hour | §7 |
